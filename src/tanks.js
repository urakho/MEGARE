...
function showTankDetail(tankType) {
    ...
    if (trioEl) {
        ...
        if (trio) {
            const c = trio.color;
            // Build member chips: emoji + short name, highlight current tank
            const chips = trio.members.map(([mType, mIcon]) => {
                ...
                return `<span style="
                    ...
                    background:${isActive ? c + '33' : 'rgba(255,255,255,0.06)'};
                    border:1px solid ${isActive ? c : 'rgba(255,255,255,0.15)'};
                    border-radius:20px; padding:3px 9px 3px 7px;
                    font-size:12px; color:${isActive ? '#fff' : '#ccc'};
