...
if (ADMIN_ENABLED !== 'on') {
    ...
    function processDevCommand(rawCommand) {
        ...
        if (command.startsWith('/cash')) {
            ...
            // Preserve limited tanks (e.g. kamikaze) when clearing
            const _limited = (typeof unlockedTanks !== 'undefined' ? unlockedTanks : []).filter(t => {
                const rm = typeof tankRarityMap !== 'undefined' ? tankRarityMap : {};
                return rm[t] === 'limited';
            });
            unlockedTanks = ['normal', ..._limited]; saveProgress();
            ...
            console.log('All tanks except normal (and limited) locked.');
